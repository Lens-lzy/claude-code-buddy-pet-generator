...
// --- Serve mode (HTTP server for web UI) ---
if (opts.serve) {
  if (!isBun) {
    console.error('Error: --serve requires Bun runtime (run with: bun buddy-reroll.js --serve)')
    process.exit(1)
  }
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (url.pathname === '/check' && req.method === 'POST') {
        ...
        return new Response(JSON.stringify({ success: true, buddy: rollFull(uid) }), { headers })
      }

      if (url.pathname === '/generate' && req.method === 'POST') {
        const f = await req.json()
        const max = f.maxIterations || 500000
        const minRarityRank = f.rarity ? RARITY_RANK[f.rarity] : 0

        for (let i = 0; i < max; i++) {
          const uid = crypto.randomBytes(32).toString('hex')
          const r = rollFull(uid)

          if (f.rarity && RARITY_RANK[r.rarity] < minRarityRank) continue
          if (f.species && r.species !== f.species) continue
          if (f.eye && r.eye !== f.eye) continue
          if (f.hat && r.hat !== f.hat) continue
          if (f.shiny && !r.shiny) continue
          if (f.minStats && !Object.entries(f.minStats).every(([k, v]) => r.stats[k] >= v)) continue

          return new Response(JSON.stringify({ success: true, uid, buddy: r, iterations: i + 1 }), { headers })
        }

        return new Response(JSON.stringify({ success: false, error: 'No matching buddy found' }), { headers })
      }

      if (url.pathname === '/') {
        return new Response('Buddy Pet Generator API is running!', { headers: { 'Content-Type': 'text/plain' } })
      }
  ...
  console.log(`🎮 Buddy Pet Generator 后端已启动!`)
  console.log(`📡 服务地址: http://localhost:${server.port}`)
  console.log(`💡 请打开 buddy-generator.html 开始使用`)
  // Don't fall through to CLI search
